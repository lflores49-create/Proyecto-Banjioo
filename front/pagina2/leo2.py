...
for col in indep_vars:
    ...
    # Crear DataFrame temporal para alinear por índice y eliminar NaNs
    tmp = returns[[col, dep_var]].dropna(how='any')

    if tmp.empty:
        st.warning(f"No hay datos válidos para graficar {col} vs {dep_var}.")
        continue

    x_vals = tmp[col].to_numpy()
    y_vals = tmp[dep_var].to_numpy()

    # Esto garantiza que x_vals y y_vals tengan el mismo tamaño
    if len(x_vals) != len(y_vals):
        st.warning(f"Tamaños distintos al graficar {col} vs {dep_var}: {len(x_vals)} vs {len(y_vals)}. Se omite.")
        continue

# Seleccionar solo las dos columnas y eliminar filas con NaN
tmp = returns[[col, dep_var]].dropna(how='any')

# Extraer valores para matplotlib
x_vals = tmp[col].to_numpy()
y_vals = tmp[dep_var].to_numpy()

# Graficar
ax.scatter(x_vals, y_vals, alpha=0.5, label=col)
